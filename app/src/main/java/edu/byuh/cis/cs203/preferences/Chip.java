...
public class Chip {
     ...
     */
    public void draw(Canvas c, Paint blackLine, Resources res) {
        // set colors from preferences
        GOLD_LEAF.setColor(GameView.theme.getGOLD_LEAF());
        DARKCHIP.setColor(GameView.theme.getDarkChip());
        LIGHTCHIP.setColor(GameView.theme.getLightChip());

        // if the player want to use bitmap instead of color
        if(DARKCHIP.getColor() == 0 && LIGHTCHIP.getColor() == 0)
        {
            Bitmap light = BitmapFactory.decodeResource(res, GameView.theme.getLightChipPicture());
            Bitmap dark = BitmapFactory.decodeResource(res, GameView.theme.getDarkChipPicture());
            light = Bitmap.createScaledBitmap(light, (int)currentLocation.width(), (int)currentLocation.height(), true);
            dark = Bitmap.createScaledBitmap(dark, (int)currentLocation.width(), (int)currentLocation.height(), true);
            if (selected) {
                c.drawCircle(currentLocation.centerX(), currentLocation.centerY(),
                        currentLocation.width()*0.6f, GOLD_LEAF);
            }
            if (getColor() == Team.DARK) {
                c.drawBitmap(dark, currentLocation.left, currentLocation.top, null);
            } else {
                c.drawBitmap(light, currentLocation.left, currentLocation.top, null);
            }
            c.drawCircle(currentLocation.centerX(), currentLocation.centerY(),
     ...
     */
    public void setDestination(Cell c,float speed) {
        ...
        velocity.x = dir.x * currentLocation.width() * speed;
        velocity.y = dir.y * currentLocation.width() * speed;
    }
     ...
     */
    public void animate(boolean isAnime) {
        if (isMoving()) {
            if (!isAnime) {
                setCell(destination);
            }
